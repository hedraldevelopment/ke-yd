...
class keyd_node:           # class representing a user using the encryption library
    ...
    def __init__(self, private_key=None, base=DEFAULT_BASE, modulus=DEFAULT_MODULUS, epoch_scale=DEFAULT_EPOCH_SCALE):
    ...
    def init_exchange(self, other_public_key): # initialise an exchange with another person
        self.exchange_key_dictionary.update(   # add to the exchange key dictionary
            {other_public_key : keygen.gen_exchange_key_(other_public_key,self.private_key,self.modulus)}  # generate an exchange key from the other public key
            )
